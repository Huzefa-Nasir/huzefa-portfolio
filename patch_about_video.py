...
new='''<div class="about-film-shell reveal-scale"><div class="about-film-top"><span>2024 → 2026</span><b>Automation story · 13 sec</b></div><div class="about-film-stage"><div class="about-film-fallback"><strong>100+</strong><span>AUTOMATIONS BUILT</span></div><video id="aboutFilm" muted loop playsinline autoplay preload="none" aria-label="Animated story showing Huzefa's automation work growing from one workflow to more than one hundred"><source data-src="about-automation-story.mp4" type="video/mp4"></video><div class="about-film-badge"><i></i><span>Built as a motion piece — not a stock reel</span></div></div><div class="about-film-foot"><span>From one trigger to 100+ automations.</span><strong>Build · debug · automate</strong></div></div>'''
if old in s:
    s=s.replace(old,new,1)
elif 'id="aboutFilm"' not in s:
    raise SystemExit('About stats/video block not found')

css='''
/* Premium About motion film */
.about-film-shell{border:1px solid var(--line);border-radius:22px;overflow:hidden;background:#101012;box-shadow:0 24px 70px #0005;align-self:center}.about-film-top,.about-film-foot{display:flex;align-items:center;justify-content:space-between;gap:14px;padding:13px 15px;background:#111013}.about-film-top{border-bottom:1px solid #ffffff0c}.about-film-foot{border-top:1px solid #ffffff0c}.about-film-top span{font:700 9px 'Space Grotesk';letter-spacing:.14em;color:var(--o)}.about-film-top b,.about-film-foot span{font-size:10px;color:#9e9794}.about-film-foot strong{font:700 10px 'Space Grotesk';color:#fff;text-transform:uppercase;letter-spacing:.06em}.about-film-stage{position:relative;aspect-ratio:16/9;background:radial-gradient(circle at 50% 45%,#ff4f0014,transparent 38%),linear-gradient(#ffffff07 1px,transparent 1px),linear-gradient(90deg,#ffffff07 1px,transparent 1px),#09090a;background-size:auto,34px 34px,34px 34px,auto;overflow:hidden}.about-film-stage video{position:relative;z-index:2;display:block;width:100%;height:100%;object-fit:cover;background:transparent}.about-film-fallback{position:absolute;z-index:1;inset:0;display:grid;place-content:center;text-align:center}.about-film-fallback strong{font:700 clamp(54px,8vw,92px)/.9 'Space Grotesk';color:#fff;text-shadow:0 0 32px #ff4f0040}.about-film-fallback span{margin-top:12px;font:700 10px 'Space Grotesk';letter-spacing:.18em;color:var(--o)}.about-film-badge{position:absolute;z-index:3;left:14px;bottom:14px;display:flex;align-items:center;gap:8px;padding:8px 10px;border:1px solid #ffffff18;border-radius:999px;background:#09090ae8;backdrop-filter:blur(10px);font-size:9px;color:#d6d1cd}.about-film-badge i{width:7px;height:7px;border-radius:50%;background:var(--g);box-shadow:0 0 11px #b9ff6680}
@media(max-width:850px){.about-film-shell{margin-top:6px}.about-film-stage{aspect-ratio:16/9}}
@media(max-width:550px){.about-film-shell{border-radius:17px}.about-film-top,.about-film-foot{padding:10px 11px;gap:8px;align-items:flex-start}.about-film-foot{flex-direction:column}.about-film-badge{left:9px;bottom:9px;font-size:8px;padding:7px 9px}.about-film-top b{font-size:9px}.about-film-stage{background-size:auto,30px 30px,30px 30px,auto}}
@media(prefers-reduced-motion:reduce){.about-film-badge i{box-shadow:none}.about-film-stage video{display:none}}
'''
if '/* Premium About motion film */' not in s:
    s=s.replace('</style>',css+'</style>',1)

# Add autoplay to an already-integrated video for iOS/browser resilience.
s=s.replace('<video id="aboutFilm" muted loop playsinline preload="none"','<video id="aboutFilm" muted loop playsinline autoplay preload="none"',1)
...
# Ensure the explicit reveal-scale class is actually observed, otherwise it stays transparent.
s=s.replace("markReveal('#about .mini > div','reveal-scale',true);","markReveal('#about .about-film-shell','reveal-scale');",1)
if "markReveal('#about .about-film-shell','reveal-scale');" not in s:
    raise SystemExit('About film reveal observer patch failed')

p.write_text(s)
print('patched About video section + autoplay + reveal observer')
